...
Feature 1 adds `contextId` to the tap — and `RepertosaurusRepository.logPractice` already accepts it while
...
know nothing about instruments. The layer beneath does not — `RepertosaurusRepository` carries five
...
`Queries`, and one `RepertosaurusRepository.lookup(kind)`. Per-kind variation that genuinely survives —
...
- **`RepertosaurusRepository` will become a god object — split by aggregate, at feature 2.** Not yet: 330
...
- `sortName` is character-identical in `RepertosaurusRepository` and `SampleData` — decision 21's article
  rule in two places.
...
5. **`RepertosaurusRepository` never returns a SQLDelight-generated type** — every method maps to a
