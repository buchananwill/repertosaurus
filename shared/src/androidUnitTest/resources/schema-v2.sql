-- Schema version 2 — the shape that shipped with Views, before the habit arc's schema 3.
--
-- Schema-compatibility decision S13: every schema change ships with a load test against the
-- previous version's database, and schema-3 §3 names this file as that fixture. It was dumped
-- with `SELECT sql FROM sqlite_master` out of a real database the schema-2 app (commit f6b4211)
-- created on the `Pixel_3a_API_33_x86_64` emulator, so it is evidence rather than a
-- transcription. `android_metadata` is left out: it is the platform's, not the schema's.
--
-- Tables first, then indexes. Seed rows are deliberately absent — the fixture is a *shape*. The
-- rows the upgrade test needs are in `schema-v2-rows.sql`.
--
