...
public sealed interface SchemaVerdict {
    ...
    public data object NotRepertaurus : SchemaVerdict
}
...
public object SchemaCompatibility {
    ...
    public val VERSION: Long
        get() = RepertaurusDatabase.Schema.version

    /**
     * Every table and column `RepertaurusDatabase.Schema` creates.
     *
     ...
     * drawn is "not a Repertaurus database" versus "a Repertaurus database from before X", and
    ...
    public fun check(present: Map<String, Set<String>>, userVersion: Long): SchemaVerdict {
        if (!present.keys.containsAll(IDENTIFYING)) return SchemaVerdict.NotRepertaurus
    ...
    public fun explain(verdict: SchemaVerdict): String = when (verdict) {
        ...
        is SchemaVerdict.TooNew ->
            "That database was written by a newer build of Repertaurus (schema " +
                "${verdict.userVersion}, this build reads $VERSION). Update the app first."
        SchemaVerdict.NotRepertaurus ->
            "That is a SQLite file, but not a Repertaurus one."
    }
