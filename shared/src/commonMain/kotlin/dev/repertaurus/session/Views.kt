...
public object SessionViews {
     ...
     * id)` — which is the order [RepertaurusRepository.savedViews] already returns, so this
...
public class ViewCoordinator(
    ...
    public fun updateView(view: SessionView) {
        ...
        repository.updateSavedView(
            RepertaurusRepository.SavedView(
                id = view.id,
                name = display,
    ...
    private fun toSessionView(row: RepertaurusRepository.SavedView): SessionView = SessionView(
        id = row.id,
        name = row.name,
