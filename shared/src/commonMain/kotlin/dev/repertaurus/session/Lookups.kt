...
import dev.repertaurus.data.LookupRow
 ...
 * it belongs to `dev.repertaurus.data` and the layering is `session → data → core`; nothing in
...
/**
 * The one implementation, over [RepertaurusRepository]'s table-keyed lookup surface.
 *
...
public object LookupStores {
     ...
     * absent from [RepertaurusRepository.performerInstruments] and therefore has **no
     * subtitle at all** rather than an empty one. The roster still lists them.
     */
    private fun performerSubtitles(repository: RepertaurusRepository): Map<String, String> =
